defmodule GroceryPlannerWeb.DashboardLive do
  ...
  def mount(_params, _session, socket) do
    ...
    {:ok, expiring_summary} = ExpirationAlerts.get_expiring_summary(account_id, user, days_threshold: 7)
    {:ok, expiring_items} = ExpirationAlerts.get_expiring_items(account_id, user, days_threshold: 7)

    # Load recipe suggestions based on expiring items
    {:ok, recipe_suggestions} = RecipeSuggestions.get_suggestions_for_expiring_items(account_id, user, days_threshold: 7, limit: 5)
